...
const PARTY_COLORS = {
  ...
  SFF:"#92400E",LDP:"#CA8A04",CHR:"#7C3AED",
};
const pc = (code) => PARTY_COLORS[code] || "#6B7280";

export default function AnalysisPage() {
  const [districts, setDistricts] = useState([]);
  const [notionalData, setNotionalData] = useState({ current: [], compare: [] });
  ...
  // Load notional TCP data from the view (remapped to 2025 boundaries)
  useEffect(() => {
    async function loadNotional() {
      setLoading(true);
      const [{ data: current, error: e1 }, { data: compare, error: e2 }] = await Promise.all([
        supabase.from("notional_results_on_2025_boundaries")
          .select("*")
      ...
      if (e1) console.error("Current year error:", e1);
      if (e2) console.error("Compare year error:", e2);
      setNotionalData({ current: current || [], compare: compare || [] });
      setLoading(false);
    }
    loadNotional();
  }, [currentYear, compareYear]);

  // Calculate district-level TCP swings using notional data
  const districtSwings = useMemo(() => {
    if (!notionalData.current.length || !notionalData.compare.length) return [];

    const aggregate = (data) => {
      const byDistrict = {};
      data.forEach(r => {
        const d = r.district_2025;
        if (!byDistrict[d]) byDistrict[d] = {};
        byDistrict[d][r.party_code] = (byDistrict[d][r.party_code] || 0) + r.votes;
      });
      return byDistrict;
    };

    const currentAgg = aggregate(notionalData.current);
    const compareAgg = aggregate(notionalData.compare);
  ...
  const boothSwings = useMemo(() => {
    ...
    const group = (data) => {
      const g = {};
      data.forEach(r => {
        const key = `${r.district_2025}||${r.booth_name}`;
        if (!g[key]) g[key] = { district: r.district_2025, booth: r.booth_name, parties: {} };
        g[key].parties[r.party_code] = (g[key].parties[r.party_code] || 0) + r.votes;
      });
      return g;
    };

    const currGroup = group(notionalData.current);
    const compGroup = group(notionalData.compare);
    ...
    for (const [key, curr] of Object.entries(currGroup)) {
      ...
      if (currTotal < 50 || compTotal < 50) continue;

      const currALP = ((curr.parties["ALP"] || 0) / currTotal) * 100;
      const compALP = ((comp.parties["ALP"] || 0) / compTotal) * 100;

      results.push({
        district: curr.district, booth: curr.booth,
        swing: currALP - compALP, currALP, compALP, currVotes: currTotal,
      });
    }
  ...
  const stats = useMemo(() => {
    ...
    return { avg, median, toALP, fromALP, changed, total: districtSwings.length };
  }, [districtSwings]);
  ...
  const swingResults = useMemo(() => {
    ...
    for (const d of districts) {
      ...
      if (winner !== party) {
        if (winner === "ALP") results.gains.push({ name: d.district_name, from: party, margin: Math.abs(newAlpMargin).toFixed(1) });
        else results.losses.push({ name: d.district_name, to: winner, margin: Math.abs(newAlpMargin).toFixed(1) });
      }
    }
    ...
    <div style={{ padding: "20px 24px", maxWidth: 1200 }}>
      <div style={{ marginBottom: 16 }}>
        ...
        <p style={{ fontSize: 10, color: "var(--text-faint)", marginTop: 2 }}>
          All results remapped to 2025 boundaries via booth-level matching
        </p>
      </div>
      ...
      <div style={{ display: "flex", gap: 12, marginBottom: 16, alignItems: "center", flexWrap: "wrap" }}>
        ...
        {!loading && stats && (
          <span style={{ fontSize: 10, color: "var(--accent-amber)", fontFamily: "var(--font-mono)" }}>
            {stats.total}/59 districts with booth-matched data
          </span>
        )}
        ...
        <div>
          ...
          <div style={{ background: "var(--bg-surface)", border: "1px solid var(--border)", borderRadius: 6, padding: "14px 18px" }}>
            <div style={{ fontSize: 12, fontWeight: 600, color: "var(--text-primary)", marginBottom: 4 }}>
              Notional TCP Swing by District ({currentYear} vs {compareYear})
            </div>
            <div style={{ fontSize: 10, color: "var(--text-dim)", marginBottom: 12 }}>
              All results mapped to 2025 boundaries · booth-level matching
            </div>
              ...
              return (
                <div key={d.name} style={{
                  ...
                  <div style={{ position: "relative", height: 14 }}>
                    ...
                    <div style={{
                      position: "absolute", height: 10, top: 2, borderRadius: 2,
                  ...
                  <div style={{
                    fontSize: 10, fontFamily: "var(--font-mono)", fontWeight: 600, textAlign: "right",
      ...
      {tab === "calculator" && (
        <div>
          <div style={{ background: "var(--bg-surface)", border: "1px solid var(--border)", borderRadius: 6, padding: "20px 24px", marginBottom: 16 }}>
            <div style={{ fontSize: 13, fontWeight: 600, color: "var(--text-primary)", marginBottom: 4 }}>
              Uniform Swing Calculator
            </div>
            <div style={{ fontSize: 11, color: "var(--text-dim)", marginBottom: 16 }}>
              Apply a uniform TCP swing across all 59 seats based on 2025 margins.
            </div>
            ...
            <div style={{ display: "flex", gap: 16, justifyContent: "center", marginBottom: 12, flexWrap: "wrap" }}>
              {[
                ...
                <div key={p.party} style={{ textAlign: "center" }}>
                  <div style={{ fontSize: 28, fontWeight: 700, fontFamily: "var(--font-mono)", color: p.color }}>{p.count}</div>
            ...
            <div style={{ fontSize: 9, color: "var(--text-faint)", textAlign: "center", marginTop: 4 }}>Majority: 30</div>
          </div>

          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
            <div style={{ background: "var(--bg-surface)", border: "1px solid var(--border)", borderRadius: 6, padding: "14px 16px" }}>
              <div style={{ fontSize: 12, fontWeight: 600, color: "#B91C1C", marginBottom: 8 }}>ALP Gains ({swingResults.gains.length})</div>
              {swingResults.gains.length === 0 ? (
                <div style={{ fontSize: 11, color: "var(--text-dim)" }}>No gains at this swing</div>
              ) : swingResults.gains.map(g => (
                <div key={g.name} style={{ display: "flex", justifyContent: "space-between", padding: "4px 0", borderBottom: "1px solid var(--bg-base)", fontSize: 11 }}>
                  <span style={{ color: "var(--text-secondary)" }}>{g.name}</span>
                  <span style={{ fontFamily: "var(--font-mono)", fontSize: 10, color: "var(--text-dim)" }}>from {g.from} · {g.margin}%</span>
                </div>
              ))}
            </div>
            <div style={{ background: "var(--bg-surface)", border: "1px solid var(--border)", borderRadius: 6, padding: "14px 16px" }}>
              <div style={{ fontSize: 12, fontWeight: 600, color: "#1D4ED8", marginBottom: 8 }}>ALP Losses ({swingResults.losses.length})</div>
              {swingResults.losses.length === 0 ? (
                ...
                <div key={l.name} style={{ display: "flex", justifyContent: "space-between", padding: "4px 0", borderBottom: "1px solid var(--bg-base)", fontSize: 11 }}>
                  <span style={{ color: "var(--text-secondary)" }}>{l.name}</span>
                  <span style={{ fontFamily: "var(--font-mono)", fontSize: 10, color: "var(--text-dim)" }}>to {l.to} · {l.margin}%</span>
                </div>
              ))}
            </div>
          </div>
      ...
      {tab === "booths" && !loading && (
        <div>
          <div style={{ background: "var(--bg-surface)", border: "1px solid var(--border)", borderRadius: 6, padding: "14px 18px", marginBottom: 16 }}>
            <div style={{ fontSize: 12, fontWeight: 600, color: "var(--text-primary)", marginBottom: 4 }}>
              Booth-Level Swing Outliers ({currentYear} vs {compareYear})
            </div>
            <div style={{ fontSize: 11, color: "var(--text-dim)", marginBottom: 4 }}>
              Booths with the biggest TCP swings — matched across redistribution boundaries.
            </div>
            <div style={{ fontSize: 10, color: "var(--text-faint)", marginBottom: 12 }}>
              {boothSwings.length} booths matched · min 50 TCP votes · all mapped to 2025 districts
            </div>

            {[
              { title: "Top 20 Swings to ALP", color: "#B91C1C", data: boothSwings.slice(-20).reverse(), positive: true },
              { title: "Top 20 Swings from ALP", color: "#1D4ED8", data: boothSwings.slice(0, 20), positive: false },
            ].map(section => (
              <div key={section.title} style={{ marginBottom: 16 }}>
                <div style={{ fontSize: 11, fontWeight: 600, color: section.color, marginBottom: 6 }}>{section.title}</div>
                <div style={{ overflowX: "auto" }}>
                  <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 11 }}>
                    <thead>
                      <tr>
                        {["Booth", "District (2025)", `${compareYear} ALP%`, `${currentYear} ALP%`, "Swing"].map(h => (
                          <th key={h} style={{
                            padding: "5px 6px", fontSize: 9, color: "var(--text-faint)",
                            textAlign: h === "Booth" || h.includes("District") ? "left" : "right",
                    ...
                    <tbody>
                      {section.data.map((b, i) => (
                        <tr key={i} style={{ borderBottom: "1px solid var(--bg-base)" }}>
                          <td style={{ padding: "4px 6px", color: "var(--text-secondary)", fontWeight: 500, maxWidth: 180, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{b.booth}</td>
                          ...
                          <td style={{ padding: "4px 6px", textAlign: "right", fontFamily: "var(--font-mono)", fontSize: 10, fontWeight: 700, color: section.color }}>
                            {section.positive ? "+" : ""}{b.swing.toFixed(1)}
                          </td>
                        </tr>
                      ))}
                    </tbody>
